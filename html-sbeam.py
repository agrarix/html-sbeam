...
def main():
    ...
    try:
        build_time_str_ver = datetime.fromtimestamp(os.path.getmtime(__file__)).strftime('%d-%m-%Y %H:%M')
    except Exception:
        build_time_str_ver = datetime.now().strftime('%d-%m-%Y %H:%M')
        
    version = cfg["VERSION"]
    if version in ("2.0", "1.0", "1.03") or not version.startswith("("):
        version = f"({build_time_str_ver})"
    ...
    footer_tmpl = cfg.get("FOOTER", "Generated by (Agrarix)${PGM} v${VER} (${OS}) ${DATE} ${TIME}")
    if "(build: " in footer_tmpl or footer_tmpl == "${PROCESS_TIME} ${PGM} (${BUILD_TIME}) v${VER} at ${HOSTNAME}":
        footer_tmpl = "Generated by (Agrarix)${PGM} v${VER} (${OS}) ${DATE} ${TIME}"
